...
public:
    ...
    FLoadMeshAsyncAction(UObject* Owner,const FLatentActionInfo& latentInfo, const FRuntimeMeshImportParameter& params
...
class FLoadStaticMeshAsyncAction : public FPendingLatentAction
{
public:
    FName executionFunction;
    int32 outputLink;
    ...
    FLoadStaticMeshAsyncAction(UObject* Owner, const FLatentActionInfo& latentInfo, const FRuntimeMeshImportParameter& params
        , FRuntimeMeshImportExportProgressUpdateDyn progressDelegate
    ...
    virtual FString GetDescription() const override
    {
        return FString::Printf(TEXT("FLoadStaticMeshAsyncAction, loading file: %s"), *file);
    }
#endif

private:
    // Reference to the outer result of the user
    FRuntimeStaticMeshImportResult& resultRef;
    bool bTaskDone = false;
};
...
FLoadMeshAsyncAction::FLoadMeshAsyncAction(UObject* Owner, const FLatentActionInfo& latentInfo, const FRuntimeMeshImportParameter& params
        , FRuntimeMeshImportExportProgressUpdateDyn progressDelegate
        , FRuntimeMeshImportResult& result)
    ...
    , resultRef(result)
{
    ...
    URuntimeMeshImportExportLibrary::ImportScene_Async_Cpp(Owner,params, callbackFinishedRaw, callbackProgressRaw);
}
    ...
    {   
        ...
        if (latentActionManager.FindExistingAction<FLoadMeshAsyncAction>(latentInfo.CallbackTarget, latentInfo.UUID) == NULL)
        {
            latentActionManager.AddNewAction(latentInfo.CallbackTarget, latentInfo.UUID
                                             , new FLoadMeshAsyncAction(worldContextObject,latentInfo, params, progressDelegate, result));
        }
    }
}

void URuntimeMeshImportExportLibrary::ImportScene_Async_Cpp(UObject* Owner,const FRuntimeMeshImportParameter& params
...
FLoadStaticMeshAsyncAction::FLoadStaticMeshAsyncAction(UObject* Owner, const FLatentActionInfo& latentInfo, const FRuntimeMeshImportParameter& params
    , FRuntimeMeshImportExportProgressUpdateDyn progressDelegate
    , FRuntimeStaticMeshImportResult& result)
    : executionFunction(latentInfo.ExecutionFunction)
    , outputLink(latentInfo.Linkage)
    ...
    , resultRef(result)
{
    FRuntimeStaticImportFinished callbackFinishedRaw;
    callbackFinishedRaw.BindLambda([this](FRuntimeStaticMeshImportResult result) ->void {
        resultRef = MoveTemp(result);
        bTaskDone = true;
        });
    ...
    URuntimeMeshImportExportLibrary::ImportStaticMesh_Async_Cpp(Owner, params, callbackFinishedRaw, callbackProgressRaw);
}

void URuntimeMeshImportExportLibrary::ImportStaticMesh_Async(UObject* worldContextObject, FLatentActionInfo latentInfo, const FRuntimeMeshImportParameter& params, FRuntimeMeshImportExportProgressUpdateDyn progressDelegate, FRuntimeStaticMeshImportResult& result)
{
    if (UWorld* world = GEngine->GetWorldFromContextObject(worldContextObject, EGetWorldErrorMode::LogAndReturnNull))
    {
        FLatentActionManager& latentActionManager = world->GetLatentActionManager();
        if (latentActionManager.FindExistingAction<FLoadStaticMeshAsyncAction>(latentInfo.CallbackTarget, latentInfo.UUID) == NULL)
        {
            latentActionManager.AddNewAction(latentInfo.CallbackTarget, latentInfo.UUID
                , new FLoadStaticMeshAsyncAction(worldContextObject, latentInfo, params, progressDelegate, result));
        }
    }
}

void URuntimeMeshImportExportLibrary::ImportStaticMesh_Async_Cpp(UObject* Owner, const FRuntimeMeshImportParameter& params, FRuntimeStaticImportFinished callbackFinished, FRuntimeMeshImportExportProgressUpdate callbackProgress)
{
    Owner->AddToRoot();
    AsyncTask(ENamedThreads::AnyThread, [=]()-> void
        {
            FRuntimeStaticMeshImportResult* result = new FRuntimeStaticMeshImportResult();
            URuntimeMeshImportExportLibrary::ImporStaticMesh_AnyThread(Owner, params, callbackProgress, *result);
            AsyncTask(ENamedThreads::GameThread, [=]() -> void
                {
                    callbackFinished.ExecuteIfBound(MoveTemp(*result));
                    Owner->RemoveFromRoot();
...
{
    extension = extension.Replace(TEXT("."), TEXT("")); // Don't want a dot
        ...
        , FRuntimeMeshImportResult& result)
{
    ...
    RMIE_LOG(Error, "Assimp Loading File  %s", *fileFinal);
    FAssimpProgressHandler progressHandler(callbackProgress);
    Assimp::Importer importer;
    //#if PLATFORM_ANDROID
    //    Assimp::AndroidJNIIOSystem* ioSystem = new Assimp::AndroidJNIIOSystem(FAndroidApplication::GetGameActivityThis());
    //    if (nullptr != ioSystem) 
    //    {
    ...
    if (scene->HasMeshes())
    {
        //RMIE_LOG(Log,"Inside Has Meshes %s", *fileFinal);
        ...
        if (result.meshInfos.Num() > 0)
        {
            ...
            //RMIE_LOG(Log, "After Import Method %s", *fileFinal);
            ...
            //RMIE_LOG(Log, "After Import Method Section %s", *fileFinal);
        }
       ...
       // RMIE_LOG(Log, "After Normalize Scene %s", *fileFinal);
    ...
    //RMIE_LOG(Log, "After Importing Materials %s", *fileFinal);

    result.bSuccess = bMeshImportSucces && bMaterialImportSuccess;
    return;
}

void URuntimeMeshImportExportLibrary::ImporStaticMesh_AnyThread(UObject* Owner,const FRuntimeMeshImportParameter& params
        , FRuntimeMeshImportExportProgressUpdate callbackProgress
        , FRuntimeStaticMeshImportResult& result)
{
    result.bSuccess = false;
    result.mesh = nullptr;
    FRuntimeMeshImportResult result2;
    ...
    if (scene->HasMeshes())
    {
        ...
        // Import mesh data
        int32 nodeCounter = 0;
        IterateSceneNodes(scene->mRootNode, [scene, &result2, &params, &nodeCounter, &numNodes, &callbackProgress](aiNode* node)
            {
                FTransform composedNodeTransform;
                BuildComposedNodeTransform(node, composedNodeTransform);
                composedNodeTransform = composedNodeTransform * params.transform;
                ImportMeshesOfNode(scene, node, result2, composedNodeTransform);
                ++nodeCounter;
                SendProgress_AnyThread(callbackProgress, FRuntimeMeshImportExportProgress(ERuntimeMeshImportExportProgressType::ImportingMeshes, nodeCounter, numNodes));
            });

        if (result2.meshInfos.Num() > 0)
        {
            // Handle Mesh Import Methode
            switch (params.importMethodMesh)
            {
            ...
            case EImportMethodSection::Keep:
                ...
                break;
            case EImportMethodSection::Merge:
                for (FRuntimeMeshImportMeshInfo& mesh : result2.meshInfos)
                {
                    MergeAllSections(mesh.sections);
                }
                break;
        ...
        if (params.bNormalizeScene)
        {
            ...
            totalBounds.Min = FVector(0.f); // Should be initialized already to 0, but had trouble
            totalBounds.Max = FVector(0.f); // Should be initialized already to 0, but had trouble
            for (FRuntimeMeshImportMeshInfo& meshInfo : result2.meshInfos)
            {
                for (FRuntimeMeshImportSectionInfo& sectionInfo : meshInfo.sections)
                {
                    FBox currentBounds(sectionInfo.vertices);
    ...
    if (params.importMethodSection != EImportMethodSection::Merge && scene->HasMaterials())
    {
        ImportSceneMaterials(fileFinal, scene, result2, callbackProgress);
        bMaterialImportSuccess = true;
    }
    else
    {
    ...
    //UStaticMeshDescription* lMeshDescription = UStaticMesh::CreateStaticMeshDescription(Owner);

    //FMeshDescriptionBuilder MeshDescBuilder

    //MeshDescBuilder.SetMeshDescription(&lMeshDescription->GetMeshDescription());
    ...
    //MeshDescriptions.Emplace(&lMeshDescription->GetMeshDescription());
    ...
    //result.mesh = StaticMeshComponent;
